...
describe(rule, function () {
  it('should parse basic parameters', () => {
    ...
    expect(tree).to.deep.equal(new ast.FieldDefinition(new ast.Name('this'), new ast.Named(new ast.Name('string'))));
  });
  it('should parse param with annotation', () => {
    const tree = parser.parse(`this : string @hey`);
    expect(tree).to.deep.equal(
      new ast.FieldDefinition(new ast.Name('this'), new ast.Named(new ast.Name('string')), undefined, undefined, [
